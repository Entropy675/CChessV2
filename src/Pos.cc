...
{
	Pos temp(*this);
	temp += a; // utalize validity checking in +=, if invalid this fails, returning a temp with values of this (no change).
	// would be very very bad if this checking did not occur, as the we assume Pos always holds valid positions.
	return temp;
}
